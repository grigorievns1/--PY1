...
def to_csv_file(filename, headers, rows, delimiter=",", new_line="\n"):
    ...
    with open(filename, 'w', encoding='utf-8') as file:
        file.writelines(delimiter.join(headers))
        for l in rows:
            file.write(new_line)
            file.writelines(delimiter.join(l))
...
to_csv_file("output.csv", headers_list, data)# TODO вызвать функцию to_csv_file и записать данные в файл
